	    ...
	    int *inFrames, int *outFrames)
{
  ...
  for (wi = 0; wi < si->outWidth; wi++) {
    ...
    for (i = 0; i < *inFrames && i < *outFrames; i++) {
      ...
      /*
	printf("insmp = %f\n",insmp);
	printf("inputs  = (#%d - iir_in=%d)",iir->nInTaps * si->outWidth,iir_in);
      for (j = 0 ; j < iir->nInTaps * si->outWidth; j++) {
	if (j==iir_in) printf("(*)");
	printf("%.2lf\t",iir->inputs[j]);
      }
      printf("\n");
     
      printf("outputs = ");
      for (j = 0 ; j < iir->nOutTaps * si->outWidth ; j++) {
	if (j==iir_out) printf("(*)");
	printf("%.2lf\t",iir->outputs[j]);
      }
      printf("\n");
      */
	...
	*/
	...
	for (j = 0 ; j < iir->nInTaps ; j++) {
	  /*	  printf(" + %.2f * %.2f",iir->itaps[j],iir->inputs[k*si->outWidth+wi]);*/
	  outsmp += iir->itaps[j] * iir->inputs[k*si->outWidth+wi];
	  k = (k+1)%(iir->nInTaps);
	}
	//	iir_in = (iir_in+1)%(iir->nInTaps);
	iir_in = iir_in == 0 ? iir->nInTaps-1:iir_in-1;
      }
	...
	*/
	...
	for (j = 1 ; j < iir->nOutTaps ; j++) {
	  /*printf(" - %.2f * %.2f",iir->otaps[j],iir->outputs[k*si->outWidth+wi]);*/
	  outsmp -= iir->otaps[j] * iir->outputs[k*si->outWidth+wi];
	  k = (k+1)%(iir->nInTaps);
	}
	/*	printf("\n");*/
	/*	iir_out = (iir_out+1)%(iir->nOutTaps);*/
	iir_out = iir_out == 0 ? iir->nOutTaps-1:iir_out-1;
