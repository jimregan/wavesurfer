...
# An example of how to embed WaveSurfer in a sound tool built using Snack.
# See the last procedure.
...
$c create waveform 0 0 -sound snd -height $v(height) -width $v(width) -tag [list obj wave] -progress snack::progressCallback -debug $v(debug)
...
pack [button .f.ws -text OpenWS -command OpenWaveSurferWindow] -side left
...
proc OpenWaveSurferWindow {} {
    catch {destroy .ws}
    toplevel .ws
    wm title .ws "WaveSurfer window"

    set ::auto_path [concat [file dirname [pwd]parent] $::auto_path]
    package require -exact wsurf 1.8
    
    set w [wsurf .ws.ws -sound snd -collapser 0 -icons {play pause stop}]    
